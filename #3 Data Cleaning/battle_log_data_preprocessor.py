...
def parse_spells(spells_str):
    ...
    try:
        ...
        for spell_dict in spells_list:
            ...
            if isinstance(spell_dict, dict) and 'name' in spell_dict and 'icon' in spell_dict and 'nl' in spell_dict:
                if 'evolution' in spell_dict['icon']:
                    parsed_tuples.append((spell_dict['name'], spell_dict['nl'], 1))
                else:
                    parsed_tuples.append((spell_dict['name'], spell_dict['nl'], 0))
...
def parse_support_cards(support_cards_str):
    ...
    try:
        ...
        for card_dict in cards_list:
            ...
            if isinstance(card_dict, dict) and 'name' in card_dict and 'level' in card_dict and 'rarity' in card_dict:
                card_dict['level'] += rarity_increase[card_dict['rarity']]
                parsed_tuples.append((card_dict['name'], card_dict['level'], card_dict['rarity']))
